...
import stylegan_models
import stylegan_dataloader

# Make sure a directory exists for visualized samples
os.makedirs(config.sample_location, exist_ok=True)

####################
# Initialize models#
####################

generator = stylegan_models.Generator()
discriminator = stylegan_models.Discriminator()
...
try:
    pretrained = torch.load(config.load_location)
except:
    pretrained = None
...
if pretrained is None:
    ...
    # Generate sample input for visualizing training results - we only do this once, so
    # we can observe the network's behavior on the same input change over training.
    # To support style mixing, we generate one batch of input tensors for each style.
    # Values are between 0 and 1, and the tensor is BxCx1x1, where B is the number of
    # sample images we generate and C is the size of the latent space.

    # To apply the truncation trick, you could compute the magnitude of each vector and
    # replace it if it exceeds a magnitude you choose, repeating until all samples are
    # smaller than the chosen magnitude. This prevents submitting outliers, which risk
    # generating bizarre outputs, at the cost of reducing image variety.
    visualizer_samples = [torch.FloatTensor(
        numpy.random.normal(
            0, 1, (config.sample_layout[0] * config.sample_layout[1],
                   config.latent_dim,),
        )
    ).to(device=1) for s in range(config.styles_to_mix)]

    # For style mixing, it's also necessary to instruct the model where to swap styles
    # on each sample. This dataloader produces that data, indicating which AdaIN layers
    # in the model to swap styles at for each sample. Since we again want this to be
    # constant, we get one output and then never use this dataloader again.
    visualizer_style_mixer = torch.utils.data.DataLoader(
        stylegan_dataloader.StyleMixer(
            layer_count=2 * len(config.blocks),  # 2 AdaINs per block
            style_count=config.styles_to_mix,
            mixing_prob=config.style_mix_prob,
            mapping_size=config.mapping_layer_size
        ),
        batch_size=config.sample_layout[0] * config.sample_layout[1],
        num_workers=16,   # Thread count; should be about the number of CPU cores
        pin_memory=True,  # Faster data transfer to GPU
    )
    visualizer_style_mixes = next(iter(visualizer_style_mixer)).to(device=1)
else:
    ...
    visualizer_samples = pretrained["visualizer_sample"]
    visualizer_style_mixes = pretrained["visualizer_style_mixes"]
...
img_mean = torch.tensor([0.5, 0.5, 0.5], requires_grad=False).to(visualizer_samples[0].device)
img_std_dev = torch.tensor([0.5, 0.5, 0.5], requires_grad=False).to(visualizer_samples[0].device)
...
generator = stylegan_models.AccessibleDataParallel(generator, (0, 1)).cuda()
discriminator = stylegan_models.AccessibleDataParallel(discriminator, (0, 1)).cuda()

# We need to set separate learning rates for the mapping network and the rest of the
# generator - the first dict we pass the optimizer contains most parameters in the
# network and uses all the default configurations, but the second dict contains the
# mapping network parameters and a learning rate with a 1/100 multiplier to avoid
# instability. If you don't use a GPU, remove "module" from the objects below.
optimizer_G = torch.optim.Adam(
    [
        {'params': [p for p in list(generator.parameters())
                    if p not in set(generator.module.mapping_network.parameters())]},
        {'params': generator.module.mapping_network.parameters(),
         'lr': config.lr * 0.01}
...
for res_step in range(len(config.blocks)):
    ...
    for top_blocks in ([res_step - 1, res_step], [res_step]):
        ...
        resolution = config.init_res * (2 ** res_step)
        ...
        dataloader = torch.utils.data.DataLoader(
            stylegan_dataloader.FFHQDataset(
                source_directory=config.data_location,
                resize_directory=config.preprocessed_location,
            ...
            # breaks the loss computation without extra code to discard extra fakes
            drop_last=True,
        )

        # Likewise, the dataloader that instructs the model when to swap styles
        style_mixer = iter(torch.utils.data.DataLoader(
            stylegan_dataloader.StyleMixer(
                layer_count=2 * (res_step+1),  # 2 AdaINs per block
                style_count=config.styles_to_mix,
                mixing_prob=config.style_mix_prob,
                mapping_size=config.mapping_layer_size
            ),
        ...
        for epoch in range(config.n_epochs):
            for i, imgs in enumerate(dataloader):
                ...
                blend_ratio = (i + epoch * len(dataloader)) / (
                    config.n_epochs * len(dataloader)
                )
                ...
                if len(top_blocks) == 2:
                    small_imgs = torch.nn.functional.interpolate(
                        real_imgs, scale_factor=0.5, mode="bilinear"
                    )
                    small_imgs = torch.nn.functional.interpolate(
                        small_imgs, scale_factor=2.0, mode="bilinear"
                    )
                    real_imgs = (
                ...
                latent_samples = [torch.cuda.FloatTensor(
                    numpy.random.normal(0, 1, (batch_size, config.latent_dim))
                ) for s in range(config.styles_to_mix)]
                style_mixes = next(style_mixer)

                # The discriminator's training won't depend on the gradients of the
                # generator - so we can save some space for free by not storing them.
                with torch.no_grad():
                    fake_imgs = generator(latent_samples, style_mixes, top_blocks, blend_ratio)
                ...
                with torch.no_grad():
                    ...
                    visualizer.momentum_update(generator, config.visualizer_decay)

                    # Print out a variety of diagnostic information and generate some
                    # sample images every config.sample_interval image samples.
                    # Since the sample interval doesn't have to be divisible by the
                    # batch size, we're technically checking if at _least_
                    # config.sample_interval images have been shown to the model.
                    old_modulo = images_seen % config.sample_interval
                    images_seen += batch_size
                    new_modulo = images_seen % config.sample_interval
                    if new_modulo < old_modulo or config.sample_interval < batch_size:
                        print(
                        ...
                        sample_imgs = visualizer(visualizer_samples,
                                                 visualizer_style_mixes,
                                                 top_blocks,
                                                 blend_ratio)
                        ...
                        final_res = config.init_res * (
                            2 ** (len(config.blocks) - 1)
                        )
                        ...
                        torchvision.utils.save_image(
                            scaled_samples,
                            os.path.join(config.sample_location, filename),
                            nrow=config.sample_layout[0],
                            normalize=False,
                            padding=2 * len(config.blocks),
                        )
    ...
    to_save = {"generator": generator.module.state_dict(),
               ...
               "visualizer_sample": visualizer_samples,
               "visualizer_style_mixes": visualizer_style_mixes,
               "start_at": res_step + 1,
               "runtimes": runtimes,
               }
    torch.save(to_save, config.save_location)

# As a last touch, print the runtime spent at each resolution.
for resolution in range(len(config.blocks)):
    print(
        "Runtime for resolution %d: " % (config.init_res * (2 ** resolution)),
        runtimes[resolution],
    )
